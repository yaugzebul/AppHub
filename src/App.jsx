...
const App = () => {
    ...
    const tools = [
        ...
        },
        {
            ...
            url: 'https://mon-portfolio.rtestaert.fr/', // À remplacer par ton lien personnel
            color: 'bg-purple-50'
        }
    ];
        ...
        <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-24">
            ...
            <header className="bg-white border-b sticky top-0 z-10 p-4">
                <div className="max-w-md mx-auto flex justify-between items-center">
                    <div>
                        <h1 className="text-xl font-bold text-slate-800 tracking-tight">Fabrique Hub</h1>
                        <p className="text-[10px] text-slate-500 flex items-center gap-1 uppercase font-semibold">
                            <Clock size={12} /> {new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}
                        </p>
                    </div>
            ...
            <main className="max-w-md mx-auto p-4 space-y-6">
                ...
                <section>
                    <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1 text-left">Outils de formation</h2>
                            ...
                            >
                                ...
                                <div className="text-left">
                                    <div className="font-bold text-sm flex items-center gap-1">
                                        {tool.name} <ExternalLink size={10} className="text-slate-400" />
                                    </div>
                                    <div className="text-[10px] text-slate-500 font-medium">{tool.desc}</div>
                                </div>
                            </a>
                        ))}
                    </div>
                ...
                <section className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                        {notes.length === 0 && (
                            <p className="text-center text-slate-400 text-sm py-4 italic">Pas de notes pour aujourd'hui.</p>
                        )}
                        {notes.map(note => (
                            <div
                                key={note.id}
                            ...
                            >
                                ...
                                >
                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${note.completed ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300'}`}>
                                        {note.completed && <CheckCircle2 size={12} className="text-white" />}
                                    </div>
                                    <span className={`text-sm text-left ${note.completed ? 'line-through text-slate-400' : 'text-slate-700 font-medium'}`}>
                    {note.text}
                  </span>
                                </div>
                                <button
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 p-2 flex justify-around items-center max-w-md mx-auto z-20">
                <button className="p-3 text-blue-600 flex flex-col items-center gap-1">
                    <LayoutGrid size={22} />
                    <span className="text-[9px] font-bold uppercase tracking-tighter">Accueil</span>
                </button>
                ...
                <button className="p-3 text-slate-400 flex flex-col items-center gap-1">
                    <Clock size={22} />
                    <span className="text-[9px] font-bold uppercase tracking-tighter">Flux</span>
                </button>
                <button className="p-3 text-slate-400 flex flex-col items-center gap-1">
                    <User size={22} />
